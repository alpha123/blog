<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<link rel="stylesheet" href="/blog/tech/blog.css" />
			</head>
			<body>
				<a href="#main" id="skipnav">Skip to main content</a>
				<header id="banner">
					<h1><a href="/blog/tech/index.xhtml">pecan’s software musings</a></h1>
				</header>
	...
	<xsl:template name="urlForPost">
		<xsl:param name="slug" />
		<xsl:param name="date" />
		<xsl:value-of select="concat('/blog/tech/', translate($date, '-', '/'), '/', $slug, '.xhtml')" />
	</xsl:template>
	<xsl:template name="browserchrome">
		...
		<nav class="toolbar buttonsbar">
			<xsl:choose>
				<xsl:when test="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/following-sibling::posttitle[1]">
					<a aria-label="Back">
						<xsl:attribute name="href">
							<xsl:call-template name="urlForPost">
								<xsl:with-param name="slug" select="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/following-sibling::posttitle[1]/@slug" />
								<xsl:with-param name="date" select="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/following-sibling::posttitle[1]/@published" />
							</xsl:call-template>
						</xsl:attribute>
						<span>Back</span>
					</a>
			...
			<xsl:choose>
				<xsl:when test="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/preceding-sibling::posttitle[1]">
					<a aria-label="Forward">
						<xsl:attribute name="href">
							<xsl:call-template name="urlForPost">
								<xsl:with-param name="slug" select="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/preceding-sibling::posttitle[1]/@slug" />
								<xsl:with-param name="date" select="document('/blog/tech/posts.xml')//posttitle[@slug=$slug]/preceding-sibling::posttitle[1]/@published" />
							</xsl:call-template>
						</xsl:attribute>
						<span>Forward</span>
					</a>
			...
			<a href="/blog/tech/index.xhtml" aria-label="Home"><span>Home</span></a>
			<div class="separator"></div>
			<a href="https://www.google.com/search?q=site%3Aalpha123.github.io+inurl%3Ablog%2Ftech" aria-label="Search">Search</a>
			<a href="/blog/tech/favorites.xhtml" aria-label="Favorites">Favorites</a>
			<a href="/blog/tech/archive.xhtml" aria-label="History">History</a>
			<div class="separator"></div>
			<a aria-label="Mail">
				<xsl:attribute name="href">
			...
			<main id="main">
				<article>
					...
					<xsl:apply-templates select="content/node()">
						<xsl:with-param name="posts" select="'/blog/tech/posts.xml'" />
					</xsl:apply-templates>
				</article>
			</main>
	...
	<xsl:template match="page">
		...
		<div class="window active">
			<xsl:call-template name="browserchrome">
				<xsl:with-param name="title" select="title" />
				<xsl:with-param name="slug" select="@slug" />
				<xsl:with-param name="url" select="concat('/blog/tech/', @slug, '.xhtml')" />
			</xsl:call-template>
			<main id="main">
				<xsl:apply-templates select="content/node()">
